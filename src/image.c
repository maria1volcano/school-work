...
static bouleen private_valide(image,int,int);
static void    private_recopie(Comp_rgb*,int*);
static double  private_default_distance_dim3(int*,int*,bouleen);
static double  private_default_distance_dim1(int*,int*,bouleen);
...
int * image_get_pixel(image self,int x,int y)
{
  static int tab[DIM_MAX];
  int i;
  Comp_rgb* start;

  assert(private_valide(self,x,y));

  start = Debut+DEPLACEMENT(x,y);
  for(i=0;i<Dim;i++)
    tab[i] = (int)(*(start+i));
...
bouleen image_pixel_suivant(image self)
{
...
bouleen image_move_to(image self,point p)
{
  int i = COORDY(*p);
  int j = COORDX(*p);
...
bouleen image_pixel_dessus(image self)
{
...
bouleen image_pixel_dessous(image self)
{
  Comp_rgb *pixel = Courant+Longueur;
...
bouleen image_pixel_gauche(image self)
{
...
bouleen  image_pixel_droite(image self)
{
  int nb_pixel = (int) (Courant - Debut);
...
static bouleen private_valide(image self,int i,int j)
{
  bouleen valide;
